...
async def check_update(message: Message):
    """ check or do updates """
    await message.edit("`Mengecek Update, tunggu bentar telaso....`")
    ...
    if not (pull_from_repo or push_to_heroku):
        if out:
            change_log = (
                f"**Ada update baru nih asuu! [{branch}]:\n\n📄 CHANGELOG 📄**\n\n"
            )
...
async def _push_to_heroku(msg: Message, repo: Repo, branch: str) -> None:
    sent = await msg.edit(
        f"`Sedang mengupdate sekarang dari [{branch}] ke heroku...\n"
        "tunggu 5 menit dulu yaa asu`\n\n"
        f"* **Restart** sabar dulu yaa kontol `{Config.CMD_TRIGGER}restart -h`\n\n"
        "* tunggu setelah KampangUsergay sukses di update, lalu cek update kembali :)"
    )
    try:
